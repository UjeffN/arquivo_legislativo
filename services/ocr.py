    ...
    def extrair_texto_pdf(self, caminho_arquivo: str) -> str:
        ...
        try:
            ...
            if not texto_extraido.strip():
                logger.warning("Não foi possível extrair texto. Usando placeholder.")
                texto_extraido = self._gerar_texto_placeholder(caminho_arquivo)
            
            return texto_extraido
            
        except Exception as e:
            logger.error(f"Erro ao processar OCR: {e}")
            return self._gerar_texto_placeholder(caminho_arquivo)
